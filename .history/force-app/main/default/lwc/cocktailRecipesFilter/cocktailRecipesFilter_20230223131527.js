...
export default class Cocktail_recipes_filter extends LightningElement {
    ...
    handleSearchTypeChange(event){
      ...
      event = this.dispatchEvent(new CustomEvent('cocktailsearchtype',{detail: event.detail.value}));
      console.log(event);
    }

    handleSearchTerm(event){
        this.SearchTerm = event.detail.value;
      event = this.dispatchEvent(new CustomEvent('cocktailsearchterm',{detail: event.detail.value}));
      console.log(event);
